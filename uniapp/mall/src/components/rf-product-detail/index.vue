				...
				>
					<view slot="content">
						<text class="selected-text" v-if="currentSkuName === singleSkuText">
                            {{ currentCartCount }} {{ product.unit || '件' }}
                        </text>
						<text class="selected-text" v-else-if="currentSkuName">
                            {{ currentSkuName }} * {{ currentCartCount }}
                        </text>
				...
				<rf-item-popup title="积分活动" v-if="product.point_exchange_type !== '1'">
					<view slot="content" class="con-list">
						<text v-if="product.point_exchange_type">
                            兑换类型: {{ product.point_exchange_type | pointExchangeTypeFilter }}
                        </text>
						<text v-if="parseInt(product.give_point, 10) > 0">
                            赠送类型: {{ product.integral_give_type | integralGiveTypeFilter }}
                        </text>
						<text v-if="parseInt(product.give_point, 10) > 0">
                            下单可获得: {{ product | givePointFilter }}积分
                        </text>
							...
							>
								每{{ product.unit || '件' }}{{
									parseInt(
										product.ladderPreferential &&
										product.ladderPreferential[0] &&
										product.ladderPreferential[0].price,10)
								}}折</text>
						</text>
					</view>
...
<script>
     ...
     */
    import rfItemPopup from '@/components/rf-item-popup/index.vue';
