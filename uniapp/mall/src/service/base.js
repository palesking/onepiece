...
http.setConfig(config => {
	...
	// config.baseUrl = indexConfig.apphost.lizi; /* 根域名不同 */
...
async function handleRefreshToken(refresh_token) {
	...
	await http.post('/tiny-shop/v1/site/refresh', params).then(async r => {
	...
	async response => {
		...
		switch (response.data.code) {
			...
			case 401:
				...
				if (response.config.url === '/tiny-shop/v1/site/refresh') {
